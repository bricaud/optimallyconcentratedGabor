function [gamma, s, d, crit, fobj] = WinOptimBFGSOptWinFast(f, a, M, Lt, L, p, epsilon, d_init, s_init)
...
shearLat = Lt(1)/Lt(2);
shearNeg = (exp(1i * -shearLat * L / M * (t))).';

f = f .* shearNeg;
gamma=fftshift(chirplet(L, s, d)).';

c=dgt(gamma .* weight,f,a,M);
tmp1 = abs(c);
fobj = -sum(tmp1(:).^p);
