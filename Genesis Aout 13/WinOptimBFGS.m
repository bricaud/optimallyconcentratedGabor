function [gamma, s, d, crit] = WinOptimBFGS(f, a, M, Lt, L, p, epsilon)
...
%    riplot(gamma); title(['Window at loop ' num2str(iter)]);
    ...
    while (((~condition1 || ~condition2) && alpha_r - alpha_l > 1e-15 ))
    ...
    fprintf('Iteration %d: , error term d:%e, error term s:%e, crit:%e, alpha:%e, norm(grad):%e\n', iter, old_d - new_d, old_s - new_s, crit(iter), alpha_k, norm(grad));
