function [gamma, s, d, crit] = WinOptimBFGS(f, a, M, L, p, epsilon)
...
gprimd=(2*pi*t.^2/d^3-1/(2*d)).*chirplet(L,s,d);
...
for iter = 1:NbIterMax,
    ...
    while ((~condition1 || ~condition2) && alpha_r - alpha_l > 1e-10 )
        ...
        gprimd=(2*pi*t.^2/new_d^3-1/(2*new_d)).*chirplet(L,new_s,new_d);
