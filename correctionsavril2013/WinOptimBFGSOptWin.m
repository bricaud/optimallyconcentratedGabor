function [gamma, s, d, crit] = WinOptimBFGS(f, a, M, Lt, L, p, epsilon, d_init, s_init)
...
tmp1 = abs(dgt(gamma.*weight, f, a, M,'lt', Lt));
fobj = -sum(tmp1(:).^p);
...
mask = tmp1.^(p-2); 
c=dgt(gamma .* weight,f,a,M,'lt',Lt);
c=c.*mask;
%gd=gabdual(f,a,M,L,'lt',Lt);
fprocessed=idgt(c,f,a,'lt',Lt);
grad1 = (p/2) * fprocessed .*weight;
%grad1ex = (p/2) * gabmul(gamma.*weight, mask, f, a).*weight;
...
for iter = 1:NbIterMax,
    ...
    while (((~condition1 || ~condition2) && alpha_r - alpha_l > 1e-10 ))
        ...
        % new window
        new_gamma=fftshift(chirplet(L,new_s,new_d));
        new_gamma=new_gamma.';

        %%Checke the wolfe conditions
        tmp1 = dgt(new_gamma.*weight, f, a, M, 'lt', Lt);
        %tmp1 = dgt(new_gamma.*weight, f, a, M);
        tmp1 = abs(tmp1);    
        fobj = -sum(tmp1(:).^p);
        ...
        % gradient
        mask = tmp1.^(p-2); 
        %grad1 = -(p/2) * gabmul(new_gamma.*weight, mask, f, a).*weight;
        c=dgt(new_gamma.*weight,f,a,M,'lt',Lt);
        c=c.*mask;
        %gd=gabdual(f,a,M,L,'lt',Lt);
        fprocessed=idgt(c,f,a,'lt',Lt); 
        grad1 = -(p/2) * fprocessed .*weight;
    ...
    tmp1 = dgt(gamma.*weight, f, a, M, 'lt', Lt);
    tmp1 = abs(tmp1); 
